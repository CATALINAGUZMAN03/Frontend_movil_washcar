...
def get_current_user(token: str = Depends(JWTBearer()), db: Session = Depends(get_session)) -> Empleado:
    ...
    empleado = db.query(Empleado).filter(Empleado.empleado_id == user_id).first()
    if empleado is None:
...
# Función para verificar los permisos del rol
def check_permission(empleado: Empleado, modulo: str, operacion: str, db: Session):
    # Busca si el usuario tiene permiso para una operación en un módulo específico
    rol_operacion = db.query(RolOperacion).join(Operacion).join(Modulo).filter(
        RolOperacion.rol_id == empleado.rol_id,
        Modulo.nombre == modulo,
        Operacion.nombre == operacion
    ).first()
    if not rol_operacion:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="No tienes permisos para esta operacion")
    return True

def get_operations_by_role(role_id: int, db:Session):
    query = text("""
        SELECT o.nombre
        FROM operacion o
        JOIN rol_operacion ro ON o.id = ro.operacion_id
        WHERE ro.rol_id = :role_id
    """)
    results = db.execute(query, {"role_id": role_id}).fetchall()
    operation_names = [row[0] for row in results] if results else []
