...
from src.controller.roleController import (create_role, delete_role,
                                           get_role_by_id, get_roles,
                                           update_role)
from src.database.database import get_session
from src.schemas.rolSchema import Role, RoleCreate, RoleUpdate
...
@ROLE_ROUTES.put("/roles/actualizar/{role_id}", response_model=dict)
def update_role_route(role_id: int, role: RoleUpdate, db: Session = Depends(get_session)):
    return update_role(role_id, role, db)

@ROLE_ROUTES.delete("/roles/eliminar/{role_id}", response_model=dict)
